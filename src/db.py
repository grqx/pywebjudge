...
def opt_cursor(fn: Callable[Concatenate[sqlite3.Cursor, P], T]) -> Callable[P, T]:
...
@opt_cursor
def get_problems(c: sqlite3.Cursor, *, cur: OptCursor = None) -> list[sqlite3.Row]:
    return c.execute(r'SELECT id, title FROM Problem').fetchall()

@opt_cursor
def problem_info(c: sqlite3.Cursor, p_id: int, *, cur: OptCursor = None) -> sqlite3.Row | None:
    return c.execute(r'SELECT id, title, "desc" FROM Problem WHERE id = ?', (p_id, )).fetchone()

@opt_cursor
def public_testcases(c: sqlite3.Cursor, p_id: int, *, cur: OptCursor = None) -> list[sqlite3.Row]:
    return c.execute(r'SELECT test_no, "in", "out", "note" FROM Testcase WHERE problem_id = ? AND type = 0', (p_id, )).fetchall()

@opt_cursor
def creds_of(c: sqlite3.Cursor, u_name: str, *, cur: OptCursor = None) -> sqlite3.Row | None:
    return c.execute(r'SELECT user_id, pw_hash, privilege_lvl FROM User WHERE name = ?', (u_name, )).fetchone()

@opt_cursor
def register(c: sqlite3.Cursor, u_name: str, pw_hash: str, *, cur: OptCursor = None) -> None:
    c.execute(r'INSERT INTO User (name, pw_hash, privilege_lvl) VALUES (?, ?, 1)', (u_name, pw_hash))
    c.connection.commit()

@opt_cursor
def submit(c: sqlite3.Cursor, u_id: int, p_id: int, result: int, *, cur: OptCursor = None) -> None:
    c.execute(r'INSERT INTO Submission (user_id, problem_id, result) VALUES (?, ?, ?)', (u_id, p_id, result))
    c.connection.commit()

@opt_cursor
def get_subs(c: sqlite3.Cursor, u_id: int, *, cur: OptCursor = None) -> list[sqlite3.Row]:
    return c.execute(r'SELECT problem_id, result FROM Submission WHERE user_id = ?', (u_id, )).fetchall()

@opt_cursor
def get_results(c: sqlite3.Cursor, p_id: int, *, cur: OptCursor = None) -> list[sqlite3.Row]:
    return c.execute(r'SELECT result FROM Submission WHERE problem_id = ?', (p_id, )).fetchall()

@opt_cursor
def get_userinfo(c: sqlite3.Cursor, u_id: int, *, cur: OptCursor = None) -> sqlite3.Row:
